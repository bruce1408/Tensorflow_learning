...
with slim.arg_scope(nets.vgg.vgg_arg_scope()):
    logits, _ = nets.vgg.vgg_19(inputs=X, num_classes=2, dropout_keep_prob=keep_prob, is_training=True)
...
with tf.Session(config=config) as sess:
    ...
    for ep in range(EPOCH):
        ...
        print("Epoch %d: Train accuracy is %.2f; Train loss is %.5f" % (
        ep + 1, all_accuracy / (200), all_loss / (200)))

        all_accuracy = 0
        all_loss = 0
        for i in range(400):
            sess.run(valdata_init)
            accuracy_out, loss_out = sess.run([accuracy, loss], feed_dict={keep_prob: 1.0})
            all_accuracy += accuracy_out
            all_loss += loss_out
