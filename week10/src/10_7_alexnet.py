...
# 训练集
traindata = tf.data.TFRecordDataset("trainData_4_digit_nolen.tfrecord").\
    map(_parse_function).\
    repeat().shuffle(buffer_size=5000).batch(BATCHSIZE).prefetch(BATCHSIZE)
...
def conv_net(x, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope('ConvNet', reuse=reuse):
        ...
        # Convolution Layer with 32 filters and a kernel size of 3
        conv1 = tf.layers.conv2d(x, filters=64, kernel_size=11, strides=4, activation=tf.nn.relu)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        pool1 = tf.layers.max_pooling2d(conv1, 3, 2)

        # Convolution Layer with 32 filters and a kernel size of 5
        conv2_1 = tf.layers.conv2d(pool1, filters=192, kernel_size=5, padding='SAME', activation=tf.nn.relu)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        ...
        conv4 = tf.layers.conv2d(pool3, 4096, 5, activation=tf.nn.relu)
        conv4 = tf.layers.dropout(conv4, rate=dropout, training=is_training)

        conv5 = tf.layers.conv2d(conv4, 4096, 1, padding='SAME', activation=tf.nn.relu)
        conv5 = tf.layers.dropout(conv5, rate=dropout, training=is_training)
        print('conv5', conv5.shape)

        digit0 = tf.layers.conv2d(conv5, n_classes, 1, padding='SAME', activation=None)  # 这一步一直写错了，就是因为激活函数
        digit1 = tf.layers.conv2d(conv5, n_classes, 1, padding='SAME', activation=None)
        digit2 = tf.layers.conv2d(conv5, n_classes, 1, padding='SAME', activation=None)
        digit3 = tf.layers.conv2d(conv5, n_classes, 1, padding='SAME', activation=None)

        digit0 = tf.squeeze(digit0, [1, 2])
        digit1 = tf.squeeze(digit1, [1, 2])
        digit2 = tf.squeeze(digit2, [1, 2])
        digit3 = tf.squeeze(digit3, [1, 2])
...
one_hot_labels0 = tf.one_hot(indices=tf.cast(Y0, tf.int32), depth=11)
one_hot_labels1 = tf.one_hot(indices=tf.cast(Y1, tf.int32), depth=11)
one_hot_labels2 = tf.one_hot(indices=tf.cast(Y2, tf.int32), depth=11)
one_hot_labels3 = tf.one_hot(indices=tf.cast(Y3, tf.int32), depth=11)
...
loss_op = (loss_op0 *0.1 + loss_op1*0.2 + loss_op2*0.3 + loss_op3*0.4)/4.0
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
train_op = optimizer.minimize(loss_op)

# accuracy
# # label 0 is:
correct_prediction0 = tf.equal(tf.argmax(one_hot_labels0, 1), tf.argmax(logits_train_digit0, 1))
accuracy_train0 = tf.reduce_mean(tf.cast(correct_prediction0, tf.float32))

correct_prediction1 = tf.equal(tf.argmax(one_hot_labels1, 1), tf.argmax(logits_train_digit1, 1))
accuracy_train1 = tf.reduce_mean(tf.cast(correct_prediction1, tf.float32))

correct_prediction2 = tf.equal(tf.argmax(one_hot_labels2, 1), tf.argmax(logits_train_digit2, 1))
accuracy_train2 = tf.reduce_mean(tf.cast(correct_prediction2, tf.float32))

correct_prediction3 = tf.equal(tf.argmax(one_hot_labels3, 1), tf.argmax(logits_train_digit3, 1))
accuracy_train3 = tf.reduce_mean(tf.cast(correct_prediction3, tf.float32))
...
with tf.Session() as sess:
    ...
    ckpt = tf.train.get_checkpoint_state('./model_svhn10')
    if ckpt is None:
