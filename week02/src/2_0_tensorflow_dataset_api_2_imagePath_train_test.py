...
def _parse_function(imagepaths, labels):
    ...
    image_resized = tf.image.resize_images(
        image_decode, [IMG_HEIGHT, IMG_WIDTH], method=tf.image.ResizeMethod.AREA)
...
image_train, image_test, label_train, label_test = train_test_split(
    image, labels, test_size=0.25, random_state=0)
...
iterator = tf.data.Iterator.from_structure(
    traindata.output_types, traindata.output_shapes)
...
def conv_net(x, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope('ConvNet', reuse=reuse):
        ...
        conv1 = tf.layers.conv2d(
            x, filters=32, kernel_size=3, padding="same", activation=tf.nn.relu)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        conv1_1 = tf.layers.conv2d(
            conv1, filters=32, kernel_size=3, padding="same", activation=tf.nn.relu)
        conv1_2 = tf.layers.max_pooling2d(conv1_1, 2, 2)

        # Convolution Layer with 32 filters and a kernel size of 5
        conv2_1 = tf.layers.conv2d(
            conv1_2, 64, 3, padding='same', activation=tf.nn.relu)
        conv2_2 = tf.layers.conv2d(
            conv2_1, 64, 3, padding='same', activation=tf.nn.relu)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        conv2 = tf.layers.max_pooling2d(conv2_2, 2, 2)

        conv3_1 = tf.layers.conv2d(
            conv2, 128, 3, padding='same', activation=tf.nn.relu)
        conv3_2 = tf.layers.conv2d(
            conv3_1, 128, 3, padding='same', activation=tf.nn.relu)
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
