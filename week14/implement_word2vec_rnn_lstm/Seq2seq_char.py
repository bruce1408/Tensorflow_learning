...
def get_encoder_layer(input_data, rnn_size, num_layers, source_sequence_length, source_vocab_size,
    ...
    # encoder_embed_input shape is[128, 7, 15] = [batch, max_length, encoder_embed_dim]
    encoder_output, encoder_state = tf.nn.dynamic_rnn(cell, encoder_embed_input,
                                                      sequence_length=source_sequence_length, dtype=tf.float32)

    # encoder_state shape is [2 x batch x rnn_size] = [2 x 128 x 50]. 2的话是两层rnn, 每一层都有一个是c, h。

    # print('encoder_output', encoder_state[-1])
    return encoder_output, encoder_state


# Decoder 对target数据进行预处理
def process_decoder_input(data, vocab_to_int, batch_size):
    ...
    # decoder_input = tf.Print(decoder_input, [decoder_input], message='decode_input', summarize=10000)
...
def decoding_layer(target_letter_to_int, decoding_embedding_size, num_layers, rnn_size,
    ...
    print('target_letter_to_int:', target_letter_to_int.__len__())
    print('decoding_embedding_size:', decoding_embedding_size)
    print('target_sequence_length:', target_sequence_length)
    print('max_target_sequence:', max_target_sequence_length)
    print('encoder_state:\n', encoder_state)
    print('decoder_input:\n', decoder_input)
    # 1. Embedding
    target_vocab_size = len(target_letter_to_int)
    decoder_embeddings = tf.Variable(tf.random_uniform([target_vocab_size, decoding_embedding_size]))
    # decoder_embeddings = tf.Print(decoder_embeddings, [decoder_embeddings], message='embeding', summarize=10000)
    decoder_embed_input = tf.nn.embedding_lookup(decoder_embeddings, decoder_input)
    decoder_embed_input = tf.Print(decoder_embed_input, [decoder_embed_input], message='decoder_embed_input')
    ...
    with tf.variable_scope("decode", reuse=True):
        ...
        # predicting_helper = tf.Print(predicting_helper, [predicting_helper], message='prediction_helper')
...
with train_graph.as_default():
    ...
    # training_decoder_output = tf.Print(training_decoder_output.rnn_output, [training_decoder_output.rnn_output], message='train output')
    # predicting_decoder_output = tf.Print(predicting_decoder_output.sample_id, [predicting_decoder_output.sample_id],
    #                                      message='predict output')
...
(valid_targets_batch, valid_sources_batch, valid_targets_lengths, valid_sources_lengths) = \
    next(get_batches(valid_target, valid_source, batch_size, source_letter_to_int['<PAD>'],
...
with tf.Session(graph=train_graph) as sess:
    ...
    for epoch_i in range(1, epochs + 1):
        for batch_i, (targets_batch, sources_batch, targets_lengths, sources_lengths) in enumerate(
            ...
            # print('source input', sources_batch.shape)
            # print('target input', targets_batch.shape)
            # print('source length', sources_lengths)
            # print('targets length', targets_lengths.__len__())
            _, loss = sess.run([train_op, cost], {input_data: sources_batch, targets: targets_batch, lr: learning_rate,
