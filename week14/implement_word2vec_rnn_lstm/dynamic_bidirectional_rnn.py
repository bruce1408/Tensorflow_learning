...
def BiRNN(x, weights, biases):
    ...
    # outputs is a two length tuple between forward and backward
    # each outputs shape is [batch, 28, 128],states is the shape of[2, batch, 128]
    outputs, states = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, x, dtype=tf.float32)
    # try:
    #     outputs, _, _ = rnn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, x, dtype=tf.float32)
    #     # outputs, _ = rnn.rnn.bidirectional_dynamic_rnn(lstm_fw_cell, lstm_bw_cell, x, dtype=tf.float32)
    # except Exception:  # Old TensorFlow version only returns outputs not states
    #     print('old')
    #     outputs = rnn.static_bidirectional_rnn(lstm_fw_cell, lstm_bw_cell, x, dtype=tf.float32)
    # Linear activation, using rnn inner loop last output
    states = tf.concat(states, 2)  # [2, batch , 256]
