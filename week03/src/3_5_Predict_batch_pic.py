'''
Author: your name
Date: 2021-03-17 23:04:06
LastEditTime: 2021-03-17 23:06:33
LastEditors: Please set LastEditors
Description: In User Settings Edit
FilePath: /Tensorflow_learning/week02/src/2_1_predict_predict_batch_pic.py
'''
import os
import sys
import numpy as np
...
with tf.Session() as sess:
    ...
    if ckpt and ckpt.model_checkpoint_path:
        ...
        for i in natsorted(os.listdir(imgPath)):
            ...
            # 获取预测结果
            probabilities_, label = sess.run(
                [probabilities, correct_prediction], feed_dict={image: image_})
        ...
        result = pd.DataFrame.from_dict(
            data, orient='index', columns=['label'])
        result = result.reset_index().rename(columns={'index': 'id'})
        result.to_csv('./result_612_norm.csv', index=False)
